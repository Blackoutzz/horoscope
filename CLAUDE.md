...
Section 5decies (`renderAme()`), rendered in `#ame` between `.cols` and `#elements`. Three popular readings that all claim to describe "the soul", gathered in one block. Like `KBODIES`, `renderStruct()` and `renderBazi()`, **nothing here enters** `BODIES`, `dominance()`, `polarite()`, the element balance, the aspect table or the synastry index, and nothing reaches `buildPrompt()`. Nothing enters `encodeProfile()` or `chartKey()` either — both numbers derive from the birth date already in the token, so old links keep working and no cached reading is orphaned. `renderNumero()` and `#numero` are **untouched**; `renderAme(lp)` takes the life path that function already returns.

- **The age is not a second number.** `reduceFull(lifePath)` is the life path's digital root — the digital root is invariant under partial reduction, so the popular "soul number" recipe (sum all the date's digits, reduce) yields *exactly* `reduceFull(lifePath)` for every date. It is therefore rendered as a **reading of** the chemin de vie, with no seal and no digit of its own: the number appears once in the app, in `#numero`. Folding it in as a second circled figure was rejected for that reason.
- **Master numbers reduce here** (11→2, 22→4, 33→6) and the note says so before the bar. The age scale is defined on 1–9; `MASTERS` exists to stop that reduction in the life path, and `LIFEPATH[11/22/33]` writes copy on that premise. The reduction is a *declared convention*, not a competing figure — which is why it renders rather than blanking, unlike `secteDe()`, whose `null` guards a genuinely undetermined fact (a guess there is wrong half the time; a master number's root is not in doubt).
- **The bar is four named zones, not a continuum.** Every other gauge in the app runs `Exigeante → Fluide`, `Friction → Harmonie` — left is less. `LIFEPATH` treats the nine as kinds, not grades, so a life-path digit on that grammar reads as a score out of nine. Segments are proportional to band size (1 / 2 / 3 / 3); equal quarters would look tidier and misstate the mapping.
