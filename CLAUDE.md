...
| 5decies. Âge et type d'âme | ~4290 | `renderAme()` — soul age band, birthday-number instrument, solar-third origin, sunrise/sunset |
...
- **True solar time** (`eqTime`, `jdTST`, `civilTST`) — the pillars sit on local apparent time, what a sundial at the birthplace read, because that is what the tradition uses. Two terms: longitude against the zone meridian (4 min/degree, up to ~3 h in Xinjiang under China's single UTC+8) and the equation of time (±16 min). It is applied to the **UT instant**, never to zone clock time, so DST never enters — verified on a Kashgar June 1990 chart, when China ran DST at UTC+9.
  - It governs the **day boundary as well as the double-hour**. Half-applying a time standard would date a solar hour inside a zone day, and the day pillar *is* the day master — the one character the whole reading hangs on. A birth near solar midnight genuinely changes it (same instant, 戊戌 vs 己亥 by longitude alone).
  - When the corrected pillars differ from the zone-time ones, **both are shown**, the zone ones labelled as what uncorrected sites give. Same pattern as the two animal conventions: compute one, name the other. Most online calculators use zone time, so silence here reads as a bug in this app.
  - `BORD_MIN` is 4 minutes — one degree of longitude, the precision a birth certificate rounded to the minute honestly supports. Within that of solar midnight or a double-hour boundary, the block says the pillar is undetermined rather than presenting a coin-flip as fact.
  - EoT is Meeus 28.3, checked against an independently coded NOAA formulation: **max 2.97 s** over 4000 epochs 1900–2100, plus the textbook extrema (−14.2 min mid-February, +16.4 min early November) and four zero crossings. The `n360()` in it is load-bearing: `L0` is negative before 2000 and JS `%` keeps the dividend's sign, which flipped the branch and made EoT wrong **by a full day**.
- **Luck pillars (大運)** — direction is forward when the year stem's polarity and sex agree, backward otherwise; start age from the gap to the adjacent **節** at 3 days per year, shown as years and months. Eight pillars, plus the current annual pillar (流年), which turns at Lichun like the year pillar.
  - Sex is optional. Unset renders **both directions**, labelled — the unknown is one bit with two outcomes, so showing both beats choosing silently, the same move the section makes for the two calendars. The control sits in the setup form *and* in the block itself.
  - Verified by invariants, there being no external oracle: all four direction combinations; pillars stepping ±1 through the sexagenary cycle over 400 charts; and forward + backward start ages summing to the 節 gap ÷ 3. That sum is **not** exactly 10 years — it ranges ±0.5 because the sun covers 30° in 29.4 days in January and 31.4 in July. A test asserting exactly 10 would be wrong.
  - The natal pillars are relabelled (`année natale`…) before the crossing scan: « année » and « année en cours » in one sentence are unreadable together.
- Sex is the token's **9th field**. `decodeProfile` destructures, so 8-field codes still decode with `sexe:''` — old links keep working. `checkProfile` whitelists `''|h|f`, because the token is untrusted input and the value reaches the block's HTML.
- `chartKey()` hashes all 9 fields, so setting a sex changes the reading-cache key — but the reading never sees sex (nothing here reaches `buildPrompt()`). `majSexe()` therefore **migrates** the four `astro:lecture:` entries old key → new instead of orphaning them: no API call, no lost reading. The four windows all carry today's key, so they enumerate by construction — no `store.keys()`, which means it also works where `window.storage` cannot list.
...
### Type d'âme et mémoire karmique

Section 5decies (`renderAme()`), rendered in `#ame` between `.cols` and `#elements`. Three readings: karmic debts, the birthday-number instrument, the solar-third origin. Like `KBODIES`, `renderStruct()` and `renderBazi()`, **nothing here enters** `BODIES`, `dominance()`, `polarite()`, the element balance, the aspect table or the synastry index, and nothing reaches `buildPrompt()`. Nothing enters `encodeProfile()` or `chartKey()` either — everything derives from the birth date already in the token, so old links keep working and no cached reading is orphaned. `renderNumero()` and `#numero` are **untouched**; `renderAme(lp)` takes the life path that function already returns.

**A soul-age scale occupied this place and was REMOVED.** It laid the life path across four bands (naissante / jeune / mûre / ancienne) with a gauge. The life path describes the direction of *this* life — the lesson to cross — and no classical school makes it a count of incarnations; the most developed soul-age framework, the Michael Teachings, explicitly refuses date calculation and assesses by temperament. The bar therefore displayed a rank where there is no scale. **Do not reintroduce it**: no date arithmetic will give it a foundation. The `.note` says so in the page, so a reader who meets the scale elsewhere knows this app dropped it on purpose.

- **Karmic debts replaced it** (`DETTES`, `dettesDe()`). A debt appears when an **intermediate** total — before the final reduction — is 13, 14, 16 or 19: the number *crossed*, not the number arrived at. That is the whole point against the band it replaces, since reduction destroys exactly that information: two charts with life path 5 differ according to whether they passed through 14 or 23, and the old bar showed only the 5.
- **Two sources, both date-only**: the life-path total (`totalChemin()` = month, day and year reduced separately, then summed) and the day of the month. The two others the tradition cites — expression and personality — need the **full birth name**, which the app does not store; adding it would put a surname in the shareable token, hence in links. See Security: that is precisely what the fragment exists to avoid.
- **Rendered as prose, not a four-box checklist.** A checklist here would sit directly above the six `MARQ` rows and rebuild the stacked-lists problem the section already refuses twice. The calculation is shown, because the crossed total is the payload and reduction hides it.
- **65 % of dates carry no debt**, so the empty state is written copy, and it says explicitly that this means neither "new soul" nor a soul free of everything — only that this particular count says nothing about the chart. Frequencies stated in the copy (25.4 % life-path, 13.1 % day, 34.8 % at least one) are **asserted by a test**, so moving a rule without updating the numbers fails rather than lying.

- **The instrument is the birthday number** (`reduceFull(day of month)`) — genuinely independent of the life path, which takes month, day *and* year. But the source's nine archetypes duplicate `LIFEPATH`'s word for word, so `INSTRU` writes **capacities, never identity**: no archetype names, no keyword pairs, nothing mirroring `#numero`'s two-line header. About **11 %** of charts have day-root = life-path root; that case gets its own sentence (`INSTRU_MEME`) naming the convergence instead of stuttering the same archetype twice.
- **Origin: real thirds of the day and night, not clock slices.** The source cuts `00h–04h`, `04h–08h`… but "porteuse de nouvelles aubes" is a claim about the Sun, and `04h–08h` is dawn nowhere in particular — Montréal's sunrise runs 05:06 in June to 07:33 in December, both inside that one slice, and DST moves the same instant a whole slice. So daylight (sunrise→sunset) splits in three and night in three, planetary-hours style. The mapping comes out cleaner than the source's: « Stellaire » lands in actual darkness year-round, where a 00h–04h slice includes June twilight.
- `civilTST` is **not** used here, deliberately. Real sunrise and sunset instants absorb longitude and the equation of time by construction — applying the bazi section's true-solar-time shift on top would double-count both.
- **`evenementsSoleil()` takes the *nearest* solar noon**, `Math.round(jd + lon/360)`, not the noon of the UT day. At 170° of longitude the local day is over eleven hours from UT, and taking the UT day's noon put the birth outside the arc just computed — the returned third then contradicted `secteDe()`. That bug measured **5.5 %** of random charts; the fix took it to 0.5 %.
- **Invariant: the day/night half must agree with `secteDe()`** — sect is "Sun in houses 7–12", i.e. above the horizon, and the daylight arc is sunrise→sunset, i.e. above the horizon. Same binary, two computations. They agree except within the refraction offset: `angleHoraireLever` uses −0.833° (refraction + semi-diameter) while the house boundary is the geometric horizon, so a band a few minutes wide around rise and set legitimately differs. Measured over 400 random charts: 0.5 % divergence, **worst case 3.5 minutes from a boundary**. A divergence further from a boundary than that is a bug, not the offset.
- **Polar guard.** `|cos H| > 1` has no solution, so `angleHoraireLever()` returns `null`, and the block says the Sun neither rose nor set that day. It does **not** fall back to clock thirds — that would restore the very cut the section exists to avoid.
- **Untimed charts** keep the age and the instrument (date only) and lose the origin and the tie. The origin heading stays with a line saying what it needs and that it is addable in the form, plus a line stating the refusal to default: birth time is stored as `12:00` when `knownTime` is false, so a naive lookup would hand « Terrestre » to every untimed chart, confidently and silently. Do not "fix" the gap with that fallback.
- **The age × arc tie was removed with the band** — it crossed precisely what came out. Do not rebuild it against the debts: a debt is not a stage, and pairing it with the solar arc would invent a doctrine neither source holds.
